...
class MarketCrawlerWorker(ctx: Context, p: WorkerParameters) : CoroutineWorker(ctx, p) {
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        try {
            ...
            val off = p.getInt("last_offset", 0); val size = 50
            val res = fetchWithTotal(off, size)
            val tks = res.first; val total = res.second
            if (tks.isEmpty()) { p.edit().putInt("last_offset", 0).apply(); return@withContext Result.success() }
            Log.d("CRAWLER", "Processing: $off to ${off + tks.size} / $total")
            val sem = Semaphore(2)
            tks.forEach { e -> if (!isActive) return@withContext Result.retry(); sem.withPermit { ChatBotService.scrapeAndSaveEmiten(e, "crawler") } }
            p.edit().putInt("last_offset", off + size).apply(); Result.success()
        } catch (e: CancellationException) { Log.d("CRAWLER", "Task cancelled by system"); Result.retry() } catch (e: Exception) { Result.retry() }
    }
    private suspend fun fetchWithTotal(off: Int, sz: Int): Pair<List<String>, Int> = withContext(Dispatchers.IO) {
        try {
            ...
            val req = Request.Builder().url("https://query1.finance.yahoo.com/v1/finance/screener?crumb=$crumb").post(body).addHeader("Cookie", cookie ?: "").addHeader("User-Agent", "Mozilla/5.0").build()
            client.newCall(req).execute().use { r ->
                ...
                val q = j.optJSONArray("quotes") ?: return@withContext Pair(emptyList(), j.optInt("total", 0))
                val l = mutableListOf<String>(); for (i in 0 until q.length()) { val s = q.optJSONObject(i)?.optString("symbol", ""); if (s != null && s.endsWith(".JK")) l.add(s.replace(".JK", "")) }
